...
func (w *Worktree) AddWithOptions(opts *AddOptions) error {
	...
	_, err := w.doAdd(opts.Path, make([]gitignore.Pattern, 0), opts.Force)
	return err
}

// if force is true, will skip the status check
func (w *Worktree) doAdd(path string, ignorePattern []gitignore.Pattern, force bool) (plumbing.Hash, error) {
	var s Status
	var err error
